...
def run(arch_ciudades, tree=None, file=None):
    ...
    ciudades = obtener_archivos.get_archivo_ciudades(path=arch_ciudades)

    #--- Hago esto si el parametro es un archivo. tengo que dejar travel completo
    if file is not None:
        travel = obtener_archivos.get_archivo_solucion(path=file)

    #--- Hago esto si el parametro es un arbol. tengo que dejar travel completo
    if tree is not None:
        print("TODO")
